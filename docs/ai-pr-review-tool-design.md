...
### 5.3.1 大规模 PR 与批量 PR 处理

大规模场景分为两类：

- 单个大 PR：重点是文件分类、风险评分、chunk 预算、LLM 调用预算和分析覆盖率。
- 多个 PR 批量处理：重点是队列、批量总览、每个 PR 独立报告和单个失败不影响整批。

单个大 PR 不追求把所有 diff 一次性送入模型，而是先规则扫描，再按风险选择高价值 chunk 深度分析。报告必须明确展示分析覆盖率、跳过文件、跳过原因和预算限制，避免把“未发现问题”误解为“完整分析后确认安全”。

批量 PR 不应生成一个巨大混合报告，而应为每个 PR 保存独立 Review 结果，并在批量总览中展示状态、风险等级、阻塞问题数量、合并建议和覆盖率。GitHub 写回仍然必须显式开启。

详细设计见 `docs/large-and-batch-pr-handling.md`。
